...
def get_location_by_id(company_api_key,id):
    location = utils.get_location_by_id(company_api_key,id)
    return jsonify(location)

@app.route("/location/<company_api_key>", methods=["POST"])
def insert_location(company_api_key):
    location_details = request.get_json()
...
def get_sensor_by_id(company_api_key,id):
    sensor = utils.get_sensor_by_id(company_api_key, id)
...
@app.route('/sensors_data/<company_api_key>', methods=["GET"])
def get_list_sensor_data(company_api_key):
    sensor_details = request.get_json()
    list = sensor_details["list"]
    sensors = utils.get_list_sensor_data(company_api_key, list)
    return jsonify(sensors)

@app.route("/sensor_data/<company_api_key>/<id>", methods=["GET"])
def get_sensor_data_by_id(company_api_key,id):
    sensor = utils.get_sensor_data_by_id(company_api_key, id)
    return jsonify(sensor)

@app.route("/sensor_data/<company_api_key>/<id>", methods=["DELETE"])
def delete_sensor_data(company_api_key,id):
    result = utils.delete_sensor_data(company_api_key, id)
    return jsonify(result)
...
def update_sensor_data(company_api_key):
    sensor_details = request.get_json()
    id = sensor_details["id"]
    sensor_id = sensor_details["sensor_id"]
    name = sensor_details["name"]
    data = sensor_details["data"]
    date = sensor_details["date"]
    result = utils.update_sensor_data(company_api_key, id, sensor_id, name, data, date)
    return jsonify(result)
